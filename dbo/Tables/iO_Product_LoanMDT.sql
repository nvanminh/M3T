CREATE TABLE [dbo].[iO_Product_LoanMDT] (
    ...
    [RLM_IDLink_SourceOfDeposit]                VARCHAR (40)  NULL,
    [RLM_PurposeExplanation]                    VARCHAR (MAX) NULL,
    [RLM_IDLink_XRPu_L1]                        VARCHAR (40)  NULL,
    CONSTRAINT [PK_iO_Product_LoanMDT] PRIMARY KEY CLUSTERED ([RLM_ID] ASC) WITH (FILLFACTOR = 85)
);


GO
CREATE NONCLUSTERED INDEX [_DEBUG_dta_index_iO_Product_LoanMDT_25_772249856__K5_K1_6_16_78]
    ON [dbo].[iO_Product_LoanMDT]([RLM_IDLink_RMR] ASC, [RLM_ID] ASC) WITH (FILLFACTOR = 85);


GO
CREATE NONCLUSTERED INDEX [IX_iO_Product_LoanMDT]
    ON [dbo].[iO_Product_LoanMDT]([RLM_IDLink_RMR] ASC) WITH (FILLFACTOR = 85);


GO
EXECUTE sp_addextendedproperty @name = N'MS_Description', @value = N'Primary Key.', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'iO_Product_LoanMDT', @level2type = N'COLUMN', @level2name = N'RLM_ID';


GO
EXECUTE sp_addextendedproperty @name = N'MS_Description', @value = N'Redundant.', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'iO_Product_LoanMDT', @level2type = N'COLUMN', @level2name = N'RLM_Ownership';


GO
EXECUTE sp_addextendedproperty @name = N'MS_Description', @value = N'Used to compare records in Replication process.', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'iO_Product_LoanMDT', @level2type = N'COLUMN', @level2name = N'RLM_IDLink_Version';
